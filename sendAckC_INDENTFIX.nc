...
implementation {
	...
	event void AMSend.sendDone(message_t* buf,error_t err) {
		...
		if(&packet == buf && err == SUCCESS ) {
			...
			if ( call PacketAcknowledgements.wasAcked( buf ) ) {
				...
				if(TOS_NODE_ID!=8) { //if I don't receive the ack of AlertMsg and and I'm not the truck, deliver it again
					post sendAlertMsg();
				}
			}
	...
	event message_t* Receive.receive(message_t* buf,void* payload, uint8_t len) {
		...
		if (len == sizeof(alertMsg) && TOS_NODE_ID==8){//I receive an alert msg and I am the truck 
			alertMsg* mess=(alertMsg*)payload;
			tTruck=ALFABINTRUCK*sqrt((mote.positionX-mess->coord_X)*(mote.positionX-mess->coord_X)+(mote.positionY-mess->coord_Y)*(mote.positionY-mess->coord_Y));
			call TimerTruck.startOneshot(tTruck);
